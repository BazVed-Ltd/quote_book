...
RUN apt-get update -y && apt-get install -y build-essential git libvips libvips-dev libvips-tools npm \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/quote_book ./
